...
export interface GithubHttpApi {
	readonly fetch: (url: string, init?: RequestInit) => Promise<Response>
}

export class GithubHttp extends Context.Service<GithubHttp, GithubHttpApi>()(
	"@maple/api/services/vcs/vendor/github/GithubHttp",
	{
		make: Effect.sync((): GithubHttpApi => ({ fetch: (url, init) => fetch(url, init) })),
	},
) {
